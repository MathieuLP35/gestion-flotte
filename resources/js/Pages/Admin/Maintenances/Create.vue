<template>
    <Head title="Ajout d'une intervention" />
          ...
          <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 md:p-8">
              <h1 class="text-3xl font-bold text-gray-800 mb-6">
                Enregistrer une intervention (Maintenance)
              </h1>
              ...
              <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="flex items-center justify-end pt-4 border-t border-gray-200 gap-3">
                  <Link :href="route('admin.maintenances.index')" class="px-4 py-2 text-gray-600 hover:text-gray-900 font-medium">
                    Annuler
                  </Link>
...
const { vehicles, default_vehicle_id } = usePage().props

const form = useForm({
  vehicle_id: default_vehicle_id || (vehicles.length ? vehicles[0].id : null),
...
function submit() {
  form.post(route('admin.maintenances.store'), {
    onSuccess: () => form.reset('cost', 'notes', 'type'),
  })
}
</script>
      ...
      <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 grid grid-cols-2 gap-6">
          ...
          <div class="overflow-hidden shadow-sm sm:rounded-lg grid grid-cols-1 gap-6">
            ...
            <div class="p-6 md:p-8 bg-white">
              <h2 class="text-2xl font-bold text-gray-800 mb-4">Seuils de maintenance</h2>
              <div v-if="vehicle.maintenances && vehicle.maintenances.length">
                <div class="overflow-x-auto">
                  <table class="w-full text-sm text-left text-gray-500">
                    ...
                    <tbody>
                      <tr v-for="m in vehicle.maintenances" :key="m.id" class="bg-white border-b hover:bg-gray-50 text-center">
                        <td class="py-4 px-6">{{ m.km_alert_threshold }}</td>
                        <td classs="py-4 px-6">{{ formatDate(m.date_dernier_entretien) }}</td>
...
// Formulaire pour ajouter un seuil de maintenance
...
function submitSeuil() {
  formSeuil.post(route('admin.maintenances.store'), {
    onSuccess: (response) => {
      ...
      this.vehicle.maintenances = [...this.vehicle.maintenances, response.props.maintenances.slice(-1)[0]];
    },
  })
}
...
function deleteMaintenance(id) {
  if (confirm('Êtes-vous sûr de vouloir supprimer ce seuil ?')) {
    router.delete(route('admin.maintenances.destroy', id), {
      ...
      onSuccess: () => {
        this.vehicle.maintenances = this.vehicle.maintenances.filter(m => m.id !== id);
      },
    })
  }
}
